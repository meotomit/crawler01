...
def loadURL2Redis():
    '''
    query1 = "select id, site, url from visited_url_3"
    db = DB()   
    cursor = db.cursor()
    logger.info(query1)
    ...
    query1 = "SELECT DISTINCT site FROM visited_url_3"
    cursor = db.cursor()
    cursor.execute(query1)
    rows = cursor.fetchall()
    for row in rows:
    ...
    while True:
        ...
        query = "select site, url from visited_url_3 order by id limit " + str(start) + ", " + str(WINDOW_SIZE)
        logger.info(query)
        #sleep(3)
        cursor = db.cursor()
...
def insertUrl(site, url):
    query = "insert into visited_url_3 (site, url) values( '%s', '%s') " % (site, url)
    logger.info(query)
    db = DB()
    cur = db.cursor()
    cur.execute(query)
        ...
        try :
            ...
            if not existURL(className, url):
                ...
                if content:
                    ...
                    query = "INSERT INTO site_content_3 (cate_id, site, url, content, word_1, word_2, tf) values (%s, '%s', '%s', '%s', '%s', '%s', '%s')" % \
